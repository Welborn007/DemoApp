...
class IntentOfTest {
  ...
  fun intentOfTest() {
    ...
    assertThat(intent.getStringExtra("name"), `is`("weltech"))
  ...
  fun intentOfFromActionWithDataTest() {
    val intent = Intent.ACTION_MAIN.intentOf(Uri.parse("weltech")) {
      putExtra("id", userInfo.id)
      putExtra("name", userInfo.nickname)
      putExtra("position", userInfo.position)
      putExtra("poster", poster)
    ...
    assertThat(intent.action, `is`(Intent.ACTION_MAIN))
    assertThat(intent.data, `is`(Uri.parse("weltech")))
